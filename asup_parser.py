...
parser = argparse.ArgumentParser(description='process asup details into xlsx tabs and tables')
parser.add_argument('-s', '--source', help='path of directory containing files to parse')
...
def start_xml_import(filename,t_val):
    ...
    w = csv.DictWriter(out, extrasaction='ignore', delimiter='|', fieldnames=csvfieldnames, dialect=csv.QUOTE_NONE)
    
    for row in xmldict[t_val]['asup:ROW'] :
        if 'symlink_properties' in row.keys():
            if row['symlink_properties'] != None:
...
tabs = ['sis_status_l', 'cifs_server_byname', 'cifs-server-option', 'cifs_share_byname', 'cifs-share-acl', 'export_rule_table', 'volume']
tabsdetails = {'sis_status_l':
                 {'fieldnames': [
                     {'header': 'vol'},
                     {'header': 'vs'},
                     {'header': 'path'},
                     {'header': 'logical_data_size', 'total_function': 'sum', 'format': number_format},
               ...
               {'fieldnames': [
                ...
                {'fieldnames': [
                    ...
                    {'header': 'policy'},
                    {'header': 'size', 'total_function': 'sum', 'format': number_format},
                    {'header': 'avail', 'total_function': 'sum', 'format': number_format},
                    {'header': 'total', 'total_function': 'sum', 'format': number_format},
                    {'header': 'used', 'total_function': 'sum', 'format': number_format},
                    {'header': 'pcnt_used'},
                    {'header': 'files'},
                    {'header': 'files_used', 'total_function': 'sum', 'format': number_format},
                    {'header': 'maxdir_size'},
                    ...
                    {'header': 'over_provisioned_size', 'total_function': 'sum', 'format': number_format},
                    {'header': 'snap_rsrv_avail_size', 'total_function': 'sum', 'format': number_format},
...
for tab in tabs:
    ...
    worksheet.add_table(0, 0, rowcount, fieldcount, {'data': data, 'columns': fieldnames, 'total_row': True})
